...
- **Localizer** ([ws/src/hmr_localisation](ws/src/hmr_localisation)): NDT against the
  shared ground-truth map gives every robot the same global `map` frame — no
  cross-robot pose estimation needed.
- **Mapper** ([ws/src/scovox](ws/src/scovox), `scovox_mapping_node`, `mode: rolling`):
...
1. Clone recursively and build the two containers:
   ```bash
   ...
   [hmr_localisation README](ws/src/hmr_localisation/README.md) (vcs import + patch +
   colcon build; the 0.5 m localization map `gt_map/gt_map_us050.pcd` is committed).
...
3. **Extrinsics**: the static `base_link → {os_lidar, imu}` transforms baked into
   [run_localization_live.sh](ws/src/hmr_localisation/scripts/run_localization_live.sh)
...
- [scovox_fused_lidar_rgbd.yaml](ws/src/scovox/config/scovox_fused_lidar_rgbd.yaml) —
  the fused mapping base config (LiDAR owns occupancy, RGB-D semantics-only).
- [scovox_robot_share.yaml](ws/src/scovox/config/scovox_robot_share.yaml) —
  real-robot share overlay: `mode: rolling`, wall clock, change gate, 2 Hz
  coalescing, z-band [−0.5, 2.0].
- [dscovox_params.yaml](ws/src/scovox/src/scovox_mapping/config/dscovox_params.yaml) —
...
[ws/src/map_share_bandwidth_experiment.md](ws/src/map_share_bandwidth_experiment.md).

## Validating without robots

[ws/src/run_mapshare_experiment.sh](ws/src/run_mapshare_experiment.sh) replays the
recorded bag through this exact topology (N mappers + N mergers, one sensor stream
standing in for every robot) and proves cross-robot fusion: each simulated robot
shares only a disjoint z-slice, so any voxel a robot's merger holds inside a *peer's*
slice can only have arrived over the wire.
