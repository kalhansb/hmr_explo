...
- **Lead question:** how much does the exploitation phase add to the map
  beyond exploration alone — quantified per trunk and fleet-wide (§9).
...
**Exploitation ablation:** if RA-3 is reached, run it exploration-only
(`exploitation_enabled: false`, no target release). RA-1/RA-2 (exploit on)
minus RA-3 is the fleet-level exploitation-contribution measure (§9); if RA-3
is dropped for time, the per-trunk before/after measure still yields the
contribution from every exploiting run.
...
| **Exploitation map contribution** (lead question) | **per trunk:** fused-map coverage of the trunk disc at EXPLOIT-entry vs after dwell — Δ observed voxels, trunk-surface angular completeness, DBH-fit residual (EXPLOIT windows and `target_id` from the planner CSV `phase`/`target_id`/`vantage_index` + `/exploration/intents` `exploit`/`dwelled_mask`). **fleet:** RA-1/2 (exploit on) minus RA-3 (exploit off) whole-map voxel diff, concentrated within the vantage standoff of the targets |
...
| `/exploration/intents` | MinPos claims + exploit-phase timeline (`exploit` / `target_id` / `dwelled_mask`) — coordination forensics **and exploitation attribution** (shared topic, on every robot's bag) |
...
10. Exploitation-attribution build check: the exploitation-contribution
    metric (§9) needs the per-run planner CSV written (`output_csv`) **and**
    `/exploration/intents` carrying the exploit fields (`exploit`,
    `target_id`, `dwelled_mask`). Confirm `scovox_msgs` is rebuilt to the
    current `RobotIntent.msg` — the fields are absent from the stale installed
    interface, and without them the team-quota vantage sharing also degrades.
